...
export const isOnEdge = (boundingBox: Dimensions, place: [x: number, y: number], threshold: number = 10): number => {
  ...
  // right edge
  if(boundingBox.width + boundingBox.x - x >= 0 && boundingBox.width + boundingBox.x - x <= threshold && y >= boundingBox.y && y <= boundingBox.y + boundingBox.height) {
    mask |= RIGHT_EDGE;
  }
  ...
  // bottom edge
  if(boundingBox.height + boundingBox.y - y >= 0 && boundingBox.height + boundingBox.y - y <= threshold && x >= boundingBox.x && x <= boundingBox.x + boundingBox.width) {
    mask |= BOTTOM_EDGE;
  }
